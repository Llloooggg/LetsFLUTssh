...
void main() {
  group('DeepLinkHandler.parseConnectUri', () {
    test('extracts host and user', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=10.0.0.1&user=root'));
    ...
    test('extracts host, port, user — ignores credentials in URL', () {
      final config = DeepLinkHandler.parseConnectUri(
        Uri.parse('letsflutssh://connect?host=myserver.com&port=2222&user=admin&password=secret&key=id_rsa'),
      );
      expect(config, isNotNull);
    ...
    test('returns null without host', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?user=root'));
      expect(config, isNull);
    });

    test('returns null without user', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=10.0.0.1'));
      expect(config, isNull);
    });

    test('defaults port to 22 for invalid value', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h&user=u&port=abc'));
      expect(config, isNotNull);
      expect(config!.port, 22);
    });

    test('returns null for empty host', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=&user=root'));
      expect(config, isNull);
    });
    ...
    test('returns null for empty user', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h&user='));
      expect(config, isNull);
    });

    test('returns null for missing both host and user', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect'));
      expect(config, isNull);
    });

    test('defaults keyPath to empty string when not provided', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h&user=u'));
      expect(config, isNotNull);
      expect(config!.keyPath, '');
    });
    ...
    test('returns null for port 0', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h&user=u&port=0'));
      expect(config, isNull);
    });

    test('returns null for port > 65535', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h&user=u&port=70000'));
      expect(config, isNull);
    });

    test('returns null for negative port', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h&user=u&port=-1'));
      expect(config, isNull);
    });

    test('returns null for host with slash', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h/evil&user=u'));
      expect(config, isNull);
    });

    test('returns null for host with backslash', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h%5Cevil&user=u'));
      expect(config, isNull);
    });

    test('returns null for excessively long host', () {
      final longHost = 'a' * 300;
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=$longHost&user=u'));
      expect(config, isNull);
    });
    ...
    test('trims whitespace from host and user', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=%20h%20&user=%20u%20'));
      expect(config, isNotNull);
  ...
  group('DeepLinkHandler — URI classification', () {
    test('parseConnectUri returns null for empty user', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=myhost&user='));
      expect(config, isNull);
    });
    ...
    test('parseConnectUri preserves host/port/user, ignores credentials', () {
      final config = DeepLinkHandler.parseConnectUri(
        Uri.parse('letsflutssh://connect?host=h&user=u&port=8022&password=pass&key=mykey'),
      );
      expect(config, isNotNull);
      expect(config!.host, 'h');
      expect(config.user, 'u');
    ...
    test('parseConnectUri with whitespace-only host returns null', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=%20%20%20&user=u'));
      expect(config, isNull);
    });

    test('parseConnectUri with whitespace-only user returns null', () {
      final config = DeepLinkHandler.parseConnectUri(Uri.parse('letsflutssh://connect?host=h&user=%20%20'));
      expect(config, isNull);
    });

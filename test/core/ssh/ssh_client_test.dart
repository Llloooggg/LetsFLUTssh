...
void main() {
  group('SSHConfig', () {
    ...
    test('effectivePort returns 22 for zero or negative port', () {
      const config = SSHConfig(host: 'example.com', user: 'root', port: 0);
      expect(config.effectivePort, 22);

      const negConfig =
          SSHConfig(host: 'example.com', user: 'root', port: -1);
      expect(negConfig.effectivePort, 22);
    });

    test('effectivePort returns custom port', () {
      const config = SSHConfig(host: 'example.com', user: 'root', port: 2222);
      expect(config.effectivePort, 2222);
    });

    test('displayName format', () {
      const config = SSHConfig(host: 'example.com', user: 'admin', port: 22);
      expect(config.displayName, 'admin@example.com:22');
    });

    test('displayName with custom port', () {
      const config =
          SSHConfig(host: 'example.com', user: 'admin', port: 2222);
    ...
    test('hasAuth returns true with password', () {
      const config = SSHConfig(
          host: 'example.com', user: 'root', password: 'secret');
    ...
    test('copyWith preserves unchanged fields', () {
      ...
      final copy = config.copyWith(host: 'new.com');

...
void main() {
  ...
  test(
    'cast file: writer → reader roundtrip yields header + events',
    skip:
        'SessionRecorder open/close now go through the Rust core; '
        'unit-test runner has no FRB native lib loaded. Coverage '
  ...
  test(
    'encrypted file: reader rebuilds the same lines',
    skip:
        'End-to-end encryption test: writer + reader both go through '
        'lfs_core::crypto::hkdf_sha256 via FRB, which is not initialised '
        'in the flutter_test runner. Move to integration_test (or stub '
        'the FRB binding) in a follow-up. HKDF correctness is covered by '
        '`lfs_core::crypto::tests::hkdf_known_answer_test`.',
    () async {
  ...
  test(
    'readMeta returns duration + dimensions',
    skip:
        'SessionRecorder open/close now go through the Rust core; '
        'unit-test runner has no FRB native lib loaded. Coverage '
        'moves to integration_test.',

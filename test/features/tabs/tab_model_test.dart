...
void main() {
  ...
  group('TabEntry', () {
    test('copyWith changes label only', () {
      final conn = makeConn();
      final tab = TabEntry(id: 'tab-1', label: 'Original', connection: conn, kind: TabKind.sftp);
    ...
    test('copyWith without arguments keeps label', () {
      final conn = makeConn();
      final tab = TabEntry(id: 'tab-1', label: 'Original', connection: conn, kind: TabKind.terminal);
    ...
    test('duplicate creates new id, keeps connection/label/kind', () {
      final conn = makeConn();
      final tab = TabEntry(id: 'tab-1', label: 'MyTab', connection: conn, kind: TabKind.terminal);
    ...
    test('duplicate generates unique ids on each call', () {
      final conn = makeConn();
      final tab = TabEntry(id: 'tab-1', label: 'X', connection: conn, kind: TabKind.sftp);

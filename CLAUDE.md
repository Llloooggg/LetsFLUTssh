...
| **CI/CD**         | GitHub Actions: `ci.yml` on push/PR (analyze+test+SonarCloud), `build.yml` on tag or manual dispatch (preflight CI check+build+release); concurrency cancel-in-progress, Flutter SDK caching; SonarCloud (coverage QG ≥80%), CodeQL weekly scan, packaging (AppImage/deb/tar.gz, EXE/zip, dmg, per-ABI APK) |

...
- **Docs first — the highest-priority discipline, binds planning and editing both.** Docs are a first-class deliverable, not a byproduct. Every § must cover both *how it works* (mechanism, states, invariants, failure modes) **and** *why it is that way* (rationale, constraint, rejected alternative). Before any plan or edit, **consult the TOC first** ([AGENT_RULES § Within ARCHITECTURE.md](docs/AGENT_RULES.md#within-architecturemd) or the TOC at the top of `ARCHITECTURE.md`), pick the specific § that maps to the module, read that § only (plus the §§ it cross-links to). **Never read `ARCHITECTURE.md` cover-to-cover.** Then:

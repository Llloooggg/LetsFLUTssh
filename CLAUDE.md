...
| **CI/CD**         | GitHub Actions: `ci.yml` on push/PR (analyze+test), `build.yml` on tag or manual dispatch (analyze+test+build+release), SonarCloud (coverage QG ≥80%), CodeQL weekly scan, packaging (AppImage/deb/tar.gz, EXE/zip, dmg, per-ABI APK)   |

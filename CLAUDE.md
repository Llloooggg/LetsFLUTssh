...
- **Rust owns data; Flutter renders it.** All persistent state (sessions, credentials, known_hosts, config, transfer queue, snippets, tags) and all I/O (SSH, SFTP, crypto, DB, OS-keychain, biometric prompts) live in `lfs_core` / `lfs_os_security`. Flutter holds only UI state — focus, hover, scroll, dialog visibility, form drafts before submit, animation controllers. Data flow is one-way for truth: Rust → FRB → Riverpod/ChangeNotifier → widgets. **Don't cache Rust-owned data in Dart singletons / static fields / module-level `late` vars** — always re-fetch via FRB so a Rust-side mutation can't drift from a stale Dart copy. Mutations always go Rust-side; Dart sends an FRB call and re-reads, never patches a local copy.
...
**Don't escalate working baselines (unsolicited).** The project ships across 5 platforms with **deliberately uneven guarantees**. The capability ladder governs **new features and user-authorised upgrades** — it does NOT compel you to invent a 3-day native-plugin refactor in response to "fix the typo". For unsolicited proposals: check first if the user asked for this upgrade in current message or prior plan; if yes, proceed; otherwise document the gap in the per-platform table (`SECURITY.md`, §12 / §13) and don't open a refactor. Treat phrases "true X", "real X", "verified X", "proper X" as red flags when *you* feel tempted to use them to re-pitch a working feature.

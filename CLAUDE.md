...
| Edit anything under `rust/` (security/transport core) | [docs/RUST_CORE_MIGRATION_PLAN.md](docs/RUST_CORE_MIGRATION_PLAN.md) — sub-phase scope + boundary contract; [ARCHITECTURE §3.14](docs/ARCHITECTURE.md#314-rust-securitytransport-core-rust) — current state of the workspace, FRB boundary, dependency invariant. **`lfs_core` MUST NOT depend on `flutter_rust_bridge` / `tauri` / any frontend crate** — every FFI concern lives in the adapter (`lfs_frb`). Run `make rust-fmt rust-lint rust-test`; if you edited `rust/crates/lfs_frb/src/api.rs`, also `make rust-codegen` and stage the regenerated `lib/src/rust/` |

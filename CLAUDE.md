...
- **Disable vs hide unavailable controls — first on feasibility, then on surface type.** *Disable only when the capability is possible on this platform but unavailable right now* (dependency not installed, no key selected, prerequisite toggle off) — the user can act to enable it. *Hide when the capability is fundamentally impossible on the platform* (no system ssh-agent on Android/iOS, no biometric API on Linux desktop) — a permanently-greyed control the user can never enable is noise, not discoverability. Within the disable case, surface type decides presentation: *Configuration surfaces* (Settings, session-edit forms, preference dialogs) → render disabled with tooltip + tap-toast explaining the reason; *Action surfaces* (lock screen, context menus, per-row action buttons, action dialogs) → hide. Disabled state must visibly affect the whole row (opacity on full container). When hiding a config control, preserve any persisted state it maps to — hide the widget, never silently rewrite the stored value.

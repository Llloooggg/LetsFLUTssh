...
| **CI/CD**         | GitHub Actions: `ci.yml` on push/PR/dispatch (analyze+test+SonarCloud), `build.yml` on tag or dispatch (preflight CI wait+build, optional release via checkbox or auto on tag); concurrency cancel-in-progress, Flutter SDK caching; SonarCloud (coverage QG ≥80%), CodeQL weekly scan, packaging (AppImage/deb/tar.gz, EXE/zip, dmg, per-ABI APK) |

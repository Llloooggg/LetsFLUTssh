...
mod tests {
    ...
    fn record_frame_encrypted_round_trips() {
        ...
        let recording_key =
            unwrap_lfsr_header(&on_disk[..LFR_HEADER_LEN], &db_key).expect("unwrap");
        let frame_off = LFR_HEADER_LEN;
        let len =
            u32::from_le_bytes(on_disk[frame_off..frame_off + 4].try_into().unwrap()) as usize;
        assert_eq!(len, payload.len());
        let nonce = &on_disk[frame_off + 4..frame_off + 4 + NONCE_LEN];
        let ct = &on_disk[frame_off + 4 + NONCE_LEN..];
        let aad = 0u64.to_le_bytes();
        let pt = crate::crypto::aes_gcm_decrypt_raw(&recording_key[..], nonce, ct, &aad)
            .expect("decrypt");

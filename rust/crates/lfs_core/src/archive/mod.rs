...
/// failure since the archive's "created_at" is informational
/// only — the row's effective timestamp is the apply moment.
fn parse_iso8601_or_now(s: &str, now_ms: i64) -> i64 {
    if s.is_empty() {
        return now_ms;
    }
...
/// unix-millis. Howard Hinnant's algorithm.
fn civil_to_unix_ms(year: i64, month: u32, day: u32, hh: u32, mm: u32, ss: u32) -> i64 {

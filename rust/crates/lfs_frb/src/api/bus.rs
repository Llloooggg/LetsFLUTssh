...
// Connect driver entry point lives in the next 5.1 commit. The
// async-fn future generated by chaining FRB's `wrap_async` over
// `lfs_core::connection::connect_async` over `Session::connect_*_with_secret`
// trips a notorious "Send is not general enough" HRTB inference
// bug in rustc 1.85 (auto-trait propagation across `&str`-borrowing
// async fns). The fix needs an owned-arg variant family inside
// `lfs_core::ssh` so the callers don't relay `&str` through the
// FRB Send + 'static bound. Tracking in the migration plan §5.1.

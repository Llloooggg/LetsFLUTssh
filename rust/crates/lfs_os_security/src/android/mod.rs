...
//! `android.content.ClipboardManager`). The `jni` crate keeps the
//! "Rust owns OS-API" invariant on every platform without a
//! hand-maintained Kotlin shim.
//!
...
//! biometric + sensitive-clipboard runtime on Android.
//! AndroidKeyStore alias prefix + libsecret schema attributes are
//! pinned by external compat constraint — see
//! `keystore.rs::KEY_ALIAS_PREFIX`.

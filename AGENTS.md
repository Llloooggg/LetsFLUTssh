...
**Lookup discipline.** Never read `ARCHITECTURE.md` cover-to-cover. Use the `/doc <topic|§N.M>` skill (greps the heading, reads only that slice) or open the TOC and `Read` with `offset`+`limit`. A cross-link is another narrow fetch, not a full Read. `§N.M` below = look it up via /doc; bare links are kept only where the anchor is load-bearing.
...
- **Wide grep after every rule tightening.** When AGENTS.md edits remove a carve-out or expand a rule's scope, the prior plan / DoD is obsolete — what counts as a violation just changed. First follow-up = project-wide grep for every violation shape of the new rule, not just the old plan's literals. Each match migrates or gets a one-line justification. Per-commit diff review is necessary but not sufficient — agents fix only what's named; only the wide grep catches siblings.
- **Batch-mode signals override scope discipline.** A signal to finish everything end-to-end (in any language) or a handed-over multi-item plan → work the whole queue without stopping between items. Inconvenience, needed rework, or "can't QA this here" don't justify skipping, and a `.opencode/plans/` punch list is no substitute for doing the work. Stop only when the queue's empty, a real blocker hits, or the user says so.
...
- **One logical commit per arc**, not split into code/test/doc messages, unless user asks otherwise. Commit prose: no plan-IDs, no AI-tell phrasing ("I implemented…", "Let me know if…").
...
- **References are one-directional.** This file may link out to human docs; human docs and source comments must NOT link back to this file or any agent-instruction file. `.opencode/skills/*/SKILL.md` may reference this one freely. When a human doc wants to cite a rule, inline the substance.
- **Rust owns data AND logic; Flutter renders.** Every grammar, parser, validator, format-checker, decision tree, persistent state, and I/O lives in `lfs_core` / `lfs_os_security`. Flutter holds only the rendering surface — focus, hover, scroll, dialog visibility, TextField text-buffer pre-submit, animation controllers. No exceptions, no carve-outs ("form-level validation", "fast-path for common case", "test contexts that don't load FRB" etc.). If a sync FRB call hangs under `pumpAndSettle`, fix the test infrastructure (`pumpUntilFrbSettles` helper, DB / app-state bootstrap) — don't put grammar Dart-side. Data flow is one-way for truth: Rust → FRB → Riverpod/ChangeNotifier → widgets. **Don't cache Rust-owned data in Dart** (singletons, static fields, `late` vars, Riverpod NotifierProviders wrapping a Rust store) — re-fetch via FRB or subscribe to `BusEvent` stream. Mutations Rust-side only; Dart sends FRB call and re-reads, never patches locally. **The Dart `lib/core/` layer is itself Flutter-free** — it imports ZERO `package:flutter` (no UI `material`/`widgets`/`services`/`foundation`, no plugins `path_provider`/`app_links`/`flutter_foreground_task`, no `flutter_riverpod`, no l10n, no `xterm`); the only permitted framework edge is `flutter_rust_bridge`, the bridge core wraps. Everything else in `lib/core/` is pure Dart (`meta` for `@visibleForTesting`/`@immutable`, `uuid`, `path`). Placement when something in core needs a Flutter capability: app-wide UI state → `providers/`; plugin adapters (foreground service, camera, local-FS sandbox dirs, deep-link source) → `lib/platform/` or inject from the boundary; observable state core owns → a `dart:async` broadcast `Stream`, never `ChangeNotifier`/`ValueNotifier`; the support/sandbox dir → resolved once at startup and pinned Rust-side (`config_store_init` → `master_password::try_pinned_support_dir`), read back from Rust, never `path_provider` per call. Enforced by the fitness test `test/core/no_flutter_in_core_test.dart` — a stray import fails CI.
...
- **Save plans / audits / multi-axis findings to `.opencode/plans/<topic>-<YYYY-MM-DD>.md`** (gitignored), paired with a TaskList. Never hold large analyses only in chat. `docs/` is human-audience and forbids LLM asides; memory is for cross-session preferences, not project-state snapshots.
...
**Audience — write for humans, always.** `ARCHITECTURE.md`, `README.md`, `SECURITY.md`, `CONTRIBUTING.md` and every other git-tracked doc are written for humans. No LLM asides. Agent-specific guidance stays in this file. The split is absolute.
...
**Surface every improvement path — never bury it.** Notice drift / dead code / suboptimal code / weakening labels / inconsistencies — flag in one sentence ("noticed X is suboptimal because Y; want me to handle it?"). Operator decides scope — make the choice visible, then wait. **Never silently park** as "working = OK" / "low priority" / "out of scope" / "deferred" without operator confirmation, and don't unilaterally expand into a multi-day refactor. Phrases "true X" / "real X" / "verified X" / "proper X" are red flags when you use them to re-pitch a stable decision; honest drift always gets named.
...
**2. Current state only — no retrospective.** A comment describes the code *as it is now*; the "what was the bug, what is the new shape" prose belongs in the **commit message**. Before staging, grep the diff for these narrative-voice tells: `originally`, `previously`, `after the migration`, `replaces the legacy`, `now retired`, `the legacy path`, `before we`, `used to`, `Mirrors the prior`, `pre-fix`, `the prior`, `the earlier`, `legacy` (runtime sense is fine — `legacy fallback path` ok, `replaces the legacy` not).
...
- **Dialogs** — `AppDialog` for all modal dialogs. Never bare `AlertDialog`. Complex dialogs compose from `AppDialogHeader`/`AppDialogFooter`/`AppButton`. Progress: `AppProgressBarDialog.show()`. Exception: mobile touch buttons keep `Material`+`InkWell` for ripple.
- **Text selection is opt-in on desktop — clickable ≠ selectable.** No global `SelectionArea` on the desktop shell (gesture-arena race with `ThresholdDraggable`); wrap prose surfaces (dialog bodies, threat lists, release-notes, help) in `AppSelectionArea` locally, never a drag target / `AppButton` / interactive row. Mobile wraps once at `MobileShell`. Inside any `AppSelectionArea`, clickable tiles / rows / headers / badges (and form-field labels) opt out via `SelectionContainer.disabled` — `HoverRegion` auto-wraps, `InkWell` does not (wrap its child). Full mechanism + rationale: § Selection scoping.
- **Session panel input model** — focus, shortcut dispatch, two-phase folder click, and clipboard all have non-obvious contracts; read §5.3 before editing `session_panel.dart`.
...
- **Disable vs hide unavailable controls — first on feasibility, then on surface type.** *Disable only when the capability is possible on this platform but unavailable right now* (dependency not installed, no key selected, prerequisite toggle off) — the user can act to enable it. *Hide when fundamentally impossible on the platform* (no system ssh-agent on Android/iOS, no biometric API on Linux desktop) — a permanently-greyed control is noise, not discoverability. Within the disable case, surface type decides presentation: *Configuration surfaces* (Settings, session-edit forms, preference dialogs) → render disabled with tooltip + tap-toast explaining the reason; *Action surfaces* (lock screen, context menus, per-row action buttons, action dialogs) → hide. Disabled state must visibly affect the whole row (opacity on full container). When hiding a config control, preserve any persisted state it maps to — hide the widget, never silently rewrite the stored value.
...
All code follows its language's canonical style — **Dart**: Effective Dart; **Rust**: Clippy + rustfmt — and passes `make lint` with zero issues before any Dart/Rust commit. Suppression bans: see Always-On. SonarCloud gates on top, on rules the linters miss — obey these on first draft:
...
- Format per CONTRIBUTING.md → Commit messages. Drives the auto-generated GitHub release notes — keep user-readable. Use `type(scope):` with parenthesized scope for module-specific commits (`refactor(import): ...`, `test(known-hosts): ...`); drop scope only when genuinely cross-cutting.

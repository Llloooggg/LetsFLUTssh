...
# dev/scripts/agent-plan-id-gate.sh picks up immediately. Fires only
# on agent commits (Co-Authored-By: Claude trailer present);
# maintainer's own commits skip the check entirely.
ln -sf "../../dev/scripts/agent-plan-id-gate.sh" "$hook_dir/commit-msg"
chmod +x "$repo_root/dev/scripts/agent-plan-id-gate.sh"

echo "install-hooks: wrote $hook_dir/pre-commit"
echo "install-hooks: linked $hook_dir/commit-msg -> dev/scripts/agent-plan-id-gate.sh"

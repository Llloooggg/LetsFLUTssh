...
class ConfigStore {
  ...
  Future<void> save(AppConfig config) async {
    ...
    // — the actor owns the in-memory snapshot, 300 ms debounce,
    // atomic write through `write_bytes_atomic`, and the bus
    // event publication. Fallback to the inline `writeFileAtomic`
    // for flutter_test contexts that don't load the FRB native
    // lib.
    try {
      ...
      // `ConfigNotifier.update` debounce path will fold into
      // the actor when the notifier itself migrates.

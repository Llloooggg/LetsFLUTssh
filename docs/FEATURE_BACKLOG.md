...
3. **§6.1 playback browser** — list view of recordings under `<appSupport>/recordings/<sessionId>/`, decrypt-and-replay widget. Format is asciinema v2 inside the encryption envelope so a future "Export to .cast" is one decrypt away.
4. **§6.2 SSH certificates** — **upstream blocker:** dartssh2 2.17.1 has no client-side cert userauth API. Best-practice path: submit upstream PR adding cert support to `userauth_publickey.dart` + ship via local `pubspec_overrides.yaml` until it merges. Cert parser + display in key manager can ship before the upstream PR lands.
5. **§6.3 FIDO2-SSH** — desktop-first (Linux + Windows v1, macOS deferred for entitlements). Best-practice path: native HID platform channels per OS (not `package:hidapi` FFI — would break the zero-install rule on Linux distros without the package). Sequential PIN → touch dialogs.

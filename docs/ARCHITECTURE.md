...
**Class resolution from worker threads.** Android's JNI `FindClass` resolves names via the classloader of the nearest *Java* stack frame; on a worker thread attached through `attach_current_thread` with no Java frame at all — which is every FRB/tokio call site in this crate — it falls back to the system classloader, which sees only boot-classpath framework classes (`java.*`, `javax.*`, `android.*`) and cannot resolve bundled-library (`androidx.*`, `androidx.core.content.ContextCompat`) or app (`com.llloooggg.*`) classes. The failure mode is a runtime `find_class …: class not found or linkage error` even though R8 keep rules kept the class inside the APK — shrinking is not involved. `jni_helpers::load_class` therefore loads every non-framework class through the ClassLoader captured at `register()` time (`register()` runs inside `MainActivity.onCreate`, where Java frames exist and `getClassLoader()` returns the app loader); the app loader delegates up the parent chain, so framework classes resolve identically and all call sites can use one lookup path. Direct `env.find_class` remains only on boot-classpath-only modules (`keystore`, `clipboard`, parts of `hardware_vault`).

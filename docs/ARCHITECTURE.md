...
| `rsa_pkcs1_v15_sig_body(&[u8])` | Return the raw RSA PKCS#1 v1.5 signature body (identity — caller wraps once) | PKCS#11, NCrypt, TPM, Keystore RSA |
| `ed25519_sig_body(&[u8])` | Return the raw 64-byte Ed25519 signature body (length-checked; caller wraps once) | Every Ed25519 backend |
...
**Contract — every `*_sig_body` / `*_to_ssh_mpint` helper returns the bare signature *body*, never a pre-wrapped string.** The caller composes the userauth `signature` field as `string(algorithm) || string(body)` — exactly what `ssh_key::Signature` encodes. This is a trap worth naming: the RSA and Ed25519 bodies *are* the raw signature, so a helper that prepended its own `string(...)` would make the caller's wrap a double length-prefix (`string(alg) || string(string(sig))`), which a server rejects. ECDSA went unaffected because its body needs real shaping (`mpint(r) || mpint(s)`), so the bug only ever bit the raw-body algorithms — and only on hardware backends, since the software path signs through russh's own encoder. The `wire.rs::tests` assert each `body + wrap` reproduces `ssh_key::Signature`'s encoding byte-for-byte.

...
**v1 scope.** PPK v2, `ssh-ed25519`, **unencrypted + encrypted (`aes256-cbc`)**. Other variants (PPK v3, `ssh-rsa`) are recognised at parse time and surface as a typed [`PpkUnsupportedException`](#3-modules) carrying the specific dimension we don't handle yet. The importer dialog uses that to show a concrete reason rather than a generic "not a key" error.

**Why this scope first.** Modern PuTTY (0.75+, 2021) defaults to v3 + ed25519 at keygen time but legacy v2 ed25519 keys (puttygen 0.68+, 2017) are common in long-running automation, and encrypted v2 covers the typical "passphrase-protected key on a USB stick" import path. Encrypted v3 needs Argon2id KDF wiring (driven by `Argon2-Memory`/`Argon2-Passes` headers) and `ssh-rsa` needs full RSA mpint reconstruction (n = p·q, e from public, d from private) — each is a discrete commit.

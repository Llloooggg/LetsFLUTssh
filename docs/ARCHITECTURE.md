...
**v1 scope.** PPK v2, `ssh-ed25519` and `ssh-rsa`, **unencrypted + encrypted (`aes256-cbc`)**. PPK v3 is recognised at parse time and surfaces as a typed [`PpkUnsupportedException`](#3-modules) — the importer dialog uses that to show "PPK v3 not supported" rather than a generic "not a key" error.

**Algorithm coverage.** Modern PuTTY (0.75+, 2021) defaults to v3 + ed25519 at keygen time but legacy v2 keys (puttygen 0.68+ ssh-ed25519, all earlier puttygen ssh-rsa) are common in long-running automation, and encrypted v2 covers the typical "passphrase-protected key on a USB stick" import path. PPK v3 (Argon2id KDF driven by `Argon2-Memory` / `Argon2-Passes` / `Argon2-Salt` headers, HMAC-SHA-256 instead of HMAC-SHA-1) is the remaining gap; tracked as a separate commit. The codec is structured so `_parseV2` is algorithm-agnostic and `toOpenSshPem` dispatches to the per-algorithm packer ([`toOpenSshPemEd25519`](#3-modules) / [`toOpenSshPemRsa`](#3-modules)) — adding v3 means adding Argon2id KDF + HMAC-SHA-256 verification before reusing the same algorithm-specific packers.

...
**Keyboard insets handling:** To prevent terminal content from being obscured by the soft keyboard, `TerminalView` receives dynamic bottom padding: `4px + keyboardInset + keyboardBarHeight`. The `keyboardInset` is calculated as `max(0, viewInsets.bottom - bottomGap)` where `bottomGap` accounts for the bottom navigation bar height and device safe area. The `Scaffold` uses `resizeToAvoidBottomInset: false` on the terminal page to prevent xterm buffer reflow (which would cause duplicate lines when the keyboard opens), so the padding approach preserves terminal dimensions while visually shifting content up.

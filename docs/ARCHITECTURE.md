...
| `ssh_dir_import_dialog.dart` | `SshDirImportDialog` | Unified picker for `~/.ssh` contents. Two collapsible sections — "Hosts from config" (from `~/.ssh/config`, per-host checkboxes, tristate select-all) and "Keys in ~/.ssh" (scanner output, already-imported rows start unchecked with a badge). Returns one combined `ImportResult` (sessions + manager keys) that goes through the same `_applyFilteredImport` path as the .lfs archive import. Replaces the two separate dialogs this used to have |

...
3. **Scrollback eviction compensates on mobile.** xterm 4.0.0 does not correct its pixel-level `_scrollOffset` when the circular scrollback evicts the front line under `maxLines`. A user who has scrolled up (`_stickToBottom=false`) would otherwise watch the content at their current offset shift up one row per eviction, which reads as "a chunk fell out of the middle" during a paste-driven flood. `MobileTerminalView._onTerminalChanged` plugs the gap: a `CellAnchor` parked mid-buffer exposes its line's relative index via `anchor.y`, which decrements by one on every eviction (the anchor's absolute index stays fixed while `_absoluteStartIndex` climbs). The delta between two consecutive ticks is the exact eviction count, and `_compensateScrollForEviction` uses `ScrollPosition.correctBy(-evicted * cellHeight)` — the silent primitive xterm itself uses on its stick-to-bottom path — so the user's pixel position keeps pointing at the same content without interrupting an active drag. The anchor is re-parked before it drops off the front; if a single burst is large enough to evict the anchor outright, the listener conservatively compensates for `baseline + 1` (the minimum known evictions) rather than do nothing. Pinned-to-bottom users are left alone — xterm's own `_offset.correctBy` handles that path.

4. **Slow-fling physics.** `ScrollConfiguration(behavior: _SlowTerminalScrollBehavior)` wraps the `TerminalView` so xterm's internal `Scrollable` picks up `_SlowTerminalScrollPhysics` — a `ClampingScrollPhysics` subclass that caps ballistic velocity to 1200 logical px/s. A hard fling on the default physics outruns xterm's render + the eviction compensator, letting the pixel offset advance across rows xterm hasn't yet committed; the same artefact other mobile terminals (Termux et al.) resolve the same way — velocity cap on the fling. Drag velocity is not touched, so short intentional scrolls feel normal; only flicks slow down.

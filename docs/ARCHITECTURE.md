...
**UI — Forwarding tab.** `features/session_manager/session_forwards_tab.dart` is the 4th tab in the session edit dialog (Connection / Auth / Options / Forwarding) and renders **only when the active `SessionKind` is `ssh`** — WebDAV and S3 transports cannot multiplex TCP, so the tab bar drops the entry and the dialog `Stack` omits the body for those kinds. The tab owns no state — the parent dialog holds `_forwards: List<PortForwardRule>` and re-renders on `onChanged`. Edits land via the in-line `_ForwardRuleEditor` modal which validates port range / required target host before returning the rule. Persistence is deferred to the parent dialog's Save: `SaveResult.forwards` carries the in-memory list out, and `session_panel._syncForwards` diffs against the store (delete missing ids, upsert the rest) after the session row commits, so the FK constraint sees a real parent. Quick-connect / new-session paths that never touch the tab pass an empty list and skip the diff entirely.
...
The session edit dialog (`features/session_manager/session_edit_dialog.dart`) renders a `SessionKind` picker at the top of the Connection tab. Flipping kind reshapes every tab so a dialog opened on a non-SSH session never offers SSH-only controls:

- **Connection tab** — the SSH block (host / port / user / ProxyJump editor), the WebDAV block (base URL + username), or the S3 block (access key id + region + endpoint + path-style toggle + default bucket / prefix). WebDAV's auth-method picker and self-signed-cert fingerprint pin sit on the Auth tab next to the credential, not here — Connection answers "where are we connecting", Auth answers "how do we prove it".
- **Auth tab** — protocol-branched:
...
- **Forwarding tab** — only present for SSH (see the Forwarding-tab paragraph above for the visibility rule).
...
| `session_edit_dialog.dart` | `SessionEditDialog` | Create/edit session form. Dialog scaffold (build / header / tab bar / footer), state + lifecycle, and the Save pipeline in this file; per-tab UI in `session_edit_dialog_connection.dart` (kind picker + SSH host/port/user/ProxyJump *or* WebDAV base-URL/username *or* S3 access-key/region/endpoint/addressing block — every required field carries a `*` suffix on the FieldLabel for parity with the SSH `hostRequired` / `usernameRequired` ARB pattern), `session_edit_dialog_auth.dart` (protocol-branched: SSH = ssh-agent toggle + password / key store / inline-PEM / passphrase; WebDAV = method chips + credential field with label flipped to "BEARER TOKEN *" when `bearer` active + self-signed-cert fingerprint pin; S3 = single `SECRET ACCESS KEY *` field — the "Key from manager" picker renders each row's backend badge via the shared `HardwareKeyBadge` / `Pkcs11Badge` / `EnclaveBadge` / `HelloBadge` / `TpmBadge` / `KeystoreBadge` widgets so the SSH picker reads identically to the standalone key manager list), `session_edit_dialog_options.dart` (tags universally — applicable to every kind; record-session toggle only for SSH because WebDAV / S3 transports never open a shell to record). The tab bar omits the Forwarding tab whenever the active kind is not `ssh`, and `_switchKind` clamps `_tabIndex` back to Connection if a future code path ever flips `_kind` while the Forwarding tab is focused. The Auth tab label carries a kind suffix (`Auth · SSH` / `Auth · WebDAV` / `Auth · S3`) so the user notices the form below also reshapes on a kind flip — without the suffix the per-protocol auth swap was silent. `_save` rejects an invalid form with a `Toast.show(level: warning)` ("Fill the required fields marked *") plus the existing inline-error + tab-routing path, so a stray empty required field surfaces both globally and locally. Selecting "Use system ssh-agent" stamps `AuthType.agent` on the session row and clears the per-row key / password slots — the connect path reads `SshAuth.useAgent` (set by `Session.toSSHConfig` from `authType`) and short-circuits to `SshAuthAgent` inside `ConnectionsNotifier._authFromConfig` before the auth composer runs. Mobile builds keep the toggle visible but disabled — the agent endpoint is desktop-only because Android / iOS have no system ssh-agent equivalent to dial. |

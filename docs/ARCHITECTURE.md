...
- `auth_sign` maps the negotiated hash to ring's `RSA_PKCS1_SHA256` / `_SHA512`, signs in a `spawn_blocking` task (mirroring the hardware signers), and appends the SSH `signature` field `string(alg) || string(raw_sig)` to the `to_sign` buffer russh handed in — matching `ssh_key::Signature`'s RSA encoding (a single `string` wrap of the raw signature, **not** the doubly-wrapped `wire::rsa_pkcs1_v15_to_ssh_blob` shape).

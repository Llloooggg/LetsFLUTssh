...
Optional on any tier with a user-typed secret (T1+password, T2+password, Paranoid). [`BiometricAuth`](../lib/core/security/biometric_auth.dart) wraps `local_auth` for the availability probe + prompt; [`BiometricKeyVault`](../lib/core/security/biometric_key_vault.dart) stores the already-derived DB key in `flutter_secure_storage` under a platform keychain slot (iOS/macOS Keychain, Windows Credential Manager, Android EncryptedSharedPreferences). At startup, each tier's unlock path (`_unlockParanoid`, `_unlockKeychainWithPassword`, `_unlockHardware` in `main.dart`) probes `biometricKeyVault.isStored() && biometricAuth.isAvailable()` and calls `_tryBiometricUnlock()` **first**, skipping the password dialog entirely on success. Only on biometric failure / cancel does the password dialog render; when it does, it opens with `autoTriggerBiometric: false` so the user doesn't face a double-prompt, but the retry button inside the dialog stays available so they can re-invoke the system prompt without relaunching. The retry flow is shared across all three dialogs — `UnlockDialog` (Paranoid) has it natively, `TierSecretUnlockDialog` (T1+password / T2+password) accepts a `biometricUnlock` callback + `autoTriggerBiometric` flag that `main.dart` wires to the same `_biometricUnlockForTierDialog` probe.
...
**Gesture routing.** `MobileTerminalView` wraps the terminal area in a bare [`Listener`](https://api.flutter.dev/flutter/widgets/Listener-class.html) and tracks every active pointer in a `Map<int, Offset>`. One-finger events are *not* consumed by the Listener — they continue to xterm's internal gesture recognizers for scrolling + tap-to-focus. Two-finger events are handled in the Listener: the initial distance between the two pointers is snapshotted at the second pointer-down, and subsequent moves apply a proportional `fontSize` update for pinch-zoom. The font size is persisted on pointer-up through `configProvider.notifier.update`. The stock `ScaleGestureRecognizer` is deliberately **not** used — its default treats a single-pointer drag as a 1× scale and wins the gesture arena, which silently kills xterm's own recognizers sharing the same subtree.
...
**Copy mode — aim, then extend.** The overlay now has a two-phase selection model. Entering copy mode shows the virtual cursor at the current shell cursor (or viewport centre if the cursor is off-screen); the selection anchor is **not** stamped. The first single-finger drag moves the cursor freely — the "aim" phase — so the user can position the start cell without committing. On the matching pointer-up, `onAnchorDown()` fires and stamps the anchor at the current cell. Subsequent drags extend the selection from the anchor to the new cursor position; subsequent pointer-ups are no-ops because `onAnchorDown()` is idempotent (the user can lift + re-touch without losing progress). A regression that moves anchor-drop back to `pointer-down` would break the aim phase — pinned by the "first pointer-down does NOT drop a selection anchor" widget test in `mobile_terminal_view_test.dart`.

**Copy mode layout — stable-height / translate model.** The terminal widget's height **must** stay constant for the life of the session: any change propagates into `Terminal.buffer.resize`, which reshuffles scrollback lines and was the root cause of every recurring "mid-buffer gaps" / "copy-exit breaks scrollback" report. The previous "track-the-keyboard-inset" and "hint-as-Positioned-overlay" attempts each traded one regression for another. The current layout removes the tradeoff:

1. **Scaffold stays at full height.** `MobileShell` sets `resizeToAvoidBottomInset: false` on the terminal page, so the body's `constraints.maxHeight` is stable across keyboard open/close. (Other pages still shrink — they have forms that need it.)
2. **Fixed-height Column.** `MobileTerminalView` renders a `ClipRect` wrapping a `Stack` with a single `Positioned(bottom: navBarHeight + translateUp, height: bodyH - navBarHeight)`. The Column inside holds `[Expanded(terminal), SshKeyboardBar]`; terminal height is `bodyH - navBarHeight - barHeight`, constant regardless of keyboard state.
3. **Translate-up on keyboard open.** When the soft keyboard inset exceeds the mobile-shell nav bar height, `translateUp = keyboardInset - navBarHeight` pushes the whole stack up so the SSH bar sits flush against the keyboard top. The outer `ClipRect` clips the Column's overflow so translated rows never paint over the AppBar. Terminal widget **size** is untouched — only its on-screen position shifts, and xterm doesn't care about widget position, only size.
4. **Copy mode swaps bar content, not widget count.** The `SshKeyboardBar` swaps its single row's contents between the normal-keys variant and a copy-mode variant (hint text + Copy + Cancel) inside the same `Container(height: itemHeightLg)`. No banner above the terminal, no toolbar below it — no widget in the Column changes height, so `buffer.resize` never fires on toggle. The hint copy flips between "tap to mark start" and "tap to extend" driven by the overlay's `anchorSet` flag (passed down as a prop — the parent rebuilds on `onAnchorDown()` so the bar re-reads it).
5. **Overlay is visual-only.** `TerminalCopyOverlay` renders the virtual cursor marker wrapped in `IgnorePointer` (the outer `Listener` still sees cursor-pan deltas via the ancestor hit-test path); on enter it calls `TerminalController.setSuspendPointerInput(true)`. One-finger drags route through `TerminalCopyOverlayState.onCursorPan(delta)`, which accumulates sub-cell pixel deltas against the measured cell size and advances the cursor one cell at a time. When the cursor would step past the top / bottom viewport edge, `MobileTerminalView`'s shared `ScrollController` (passed to both `TerminalView(scrollController: ...)` and `TerminalCopyOverlay`) is nudged by the overflow cells' worth of pixels instead — so a single drag can extend the selection through the entire scrollback. The overlay derives the viewport-start line from `scrollController.offset ~/ cellHeight` so selection anchors stay buffer-absolute and accurate while the buffer scrolls. Horizontal overflow still clamps — xterm has no horizontal scrollback.
...
The only remaining resize trigger is orientation change (body size really does change). That is the one legitimate reason for `buffer.resize` and is unavoidable.

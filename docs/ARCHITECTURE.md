...
- **Storage access** — no runtime storage permission. Local-filesystem access goes through the Storage Access Framework via the `file_picker` plugin, which is always available and needs no `MANAGE_EXTERNAL_STORAGE` / `READ_EXTERNAL_STORAGE` grant. The previous `MANAGE_EXTERNAL_STORAGE` gate (a `com.letsflutssh/permissions` MethodChannel + an `android_storage_permission.dart` helper) was retired; `SftpInitializer.storagePermissionDenied` is now a hardcoded `false` compatibility shim. Avoiding the broad "All files access" permission also keeps the app off the Play Store's manage-storage review path.
- **Export folder picker** — `.lfs` exports pick a destination through SAF (`file_picker.getDirectoryPath`) on Android, the OS save dialog on desktop. There is no `MANAGE_EXTERNAL_STORAGE` branch any more — the gate was retired (see above), so `_pickSavePath` is a straight desktop-vs-mobile split with no all-files-access fast path

...
*Residency trade-off (explicit, not wishful thinking):* Dart `String` is immutable and GC-relocatable. There is no hook in Flutter's stock text-input pipeline to keep typed characters out of a Dart-heap `String` — the IME delivers a full `TextEditingValue` each keystroke, the framework stores `.text` on the controller, and the engine renders it. The widget mirrors the live value into a page-locked `SecretBuffer` when the caller asks for byte-level access via `onChanged`, and wipes the controller on dispose, but the short-lived interim `String`s that the framework created still live on the Dart heap until the GC runs. `SecretBuffer.fromBytes` + `wipeAndClear` at the edges is the 90% mitigation; the remaining 10% is acknowledged here so a future reader does not mistake this widget for a hermetic seal.

*Native backend (opt-in path):* [`SecureNativeTextField`](../lib/widgets/secure_native_text_field.dart) hosts a real platform text input (Android `EditText` with `TYPE_TEXT_VARIATION_PASSWORD` today; iOS / macOS / Windows / Linux planned) through a `PlatformView`. Typed bytes live in the platform's own buffer, never transit the Dart text-input pipeline, and are emitted to Dart **once** on submit as a mutable `Uint8List`. The caller copies that `Uint8List` into a `SecretBuffer` and zeroes the source in the same frame, so Dart-heap residency collapses from many per-keystroke `String` allocations to a single one-frame `Uint8List`. `SecureNativeTextField.isSupported` gates the path — unsupported platforms fall back to the stock `SecurePasswordField`. Call sites opt in per field; the regular `SecurePasswordField` is still the default because wiring `Uint8List`-based submit requires the caller to replace `TextEditingController.text` lookups with a one-shot callback.

*Native backend platform coverage:*

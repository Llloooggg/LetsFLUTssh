...
3. **Scrollback eviction compensates on mobile.** xterm 4.0.0 does not correct its pixel-level `_scrollOffset` when the circular scrollback evicts the front line under `maxLines`. A user who has scrolled up (`_stickToBottom=false`) would otherwise watch the content at their current offset shift up one row per eviction, which reads as "a chunk fell out of the middle" during a paste-driven flood. `MobileTerminalView._onTerminalChanged` plugs the gap: a `CellAnchor` parked mid-buffer exposes its line's relative index via `anchor.y`, which decrements by one on every eviction (the anchor's absolute index stays fixed while `_absoluteStartIndex` climbs). The delta between two consecutive ticks is the exact eviction count, and `_compensateScrollForEviction` nudges the shared `ScrollController` back by `evicted * cellHeight` so the user's pixel position keeps pointing at the same content. The anchor is re-parked before it drops off the front. Pinned-to-bottom users are left alone — xterm's own `_offset.correctBy` handles that path.

...
**Module layout.** The pipeline lives Rust-side in [`rust/crates/lfs_os_security/src/macos/code_signing.rs`](../rust/crates/lfs_os_security/src/macos/code_signing.rs); Dart only carries the FRB-shim adapter ([`lib/platform/macos/code_signing_client.dart`](../lib/platform/macos/code_signing_client.dart)) and the installer orchestrator ([`lib/platform/macos/installer/macos_installer.dart`](../lib/platform/macos/installer/macos_installer.dart)).
...
- **FRB shim** (`rust/crates/lfs_frb/src/api/macos_resign.rs` ↔ `lib/src/rust/api/macos_resign.dart` ↔ `lib/platform/macos/code_signing_client.dart`) — every call routes one FRB hop into `lfs_os_security`. `MacosCodeSigningClient` is a typed dispatch table for testability + DI; no business logic.
- [`MacosInstaller`](../lib/platform/macos/installer/macos_installer.dart) — silent DMG install: `hdiutil attach -nobrowse -noautoopen` → `rsync -a --delete` into `<target>.new` → `hdiutil detach` → `MacosCodeSigningClient.resignBundle(<target>.new)` (silent under existing cert) → `verifyBundle` gate → **entitlement probe** (re-extract entitlements from the staged bundle; if pre-resign had content but post-resign is empty, the re-sign silently stripped `keychain-access-groups` and we roll back). Final atomic swap: `<target>` → `<target>.backup`, then `<target>.new` → `<target>`. `.backup` is retained as a crash-recovery trail and swept by `MacosInstaller.cleanupBackup` a few seconds after the new bundle has run cleanly. The DMG / rsync subprocess shell-out stays on the Dart side because the installer also coordinates atomic-swap sequencing against `Platform.resolvedExecutable`; everything signing-related routes through the FRB shim.
...
- **Settings → Security tail row** ([`settings_sections_security._SecuritySectionState`](../lib/features/settings/settings_sections_security.dart)). Probes `MacosCodeSigningClient.hasIdentity()` once on mount and chooses between two shapes:
  ...
  - **Cert present** → destructive "Remove signing identity" button. Opens a confirmation dialog (T1 / T2 stored secrets are tied to the cert's designated requirement), then the tier-switch wizard with `capabilitiesOverride` forced to the reduced shape so the user can only pick T0 or Paranoid. `_applyTierChange` rekeys the DB under the new tier; only after the tier switch has committed does `MacosCodeSigningClient.uninstallIdentity()` delete the cert + trust entry. Order matters: removing the cert before the rekey would silently lock the user out of every T1 / T2 secret mid-flow.

- **Silent update-install callback** ([`updateServiceProvider` macOS adapter](../lib/providers/update_provider.dart)). Runs inside the download → install pipeline; `MacosInstaller.install` calls `MacosCodeSigningClient.resignBundle` silently under the already-present cert (no password prompt, `-T /usr/bin/codesign` ACL is sufficient). When no cert is present (user declined the pre-prompt and never enabled from Settings), `hasIdentity` returns `false`, the resign step is skipped, and the installed bundle keeps its CI ad-hoc signature, consistent with the user's original decline.
...
| `macosCodeSigningClientProvider` | `Provider<MacosCodeSigningClient>` | `security_provider.dart` (FRB-backed adapter for the Rust-side macOS keychain re-sign pipeline) |

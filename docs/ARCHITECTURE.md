...
**Copy mode layout.** The **Copy** button (📋 icon, right edge of the keyboard bar) toggles copy mode. `MobileTerminalView` renders a `Stack`: a top column `Positioned(bottom: bottomStripHeight)` with `CopyModeHint → Expanded(terminal + overlays)`, and a second `Positioned(bottom: viewInsets)` with a bottom strip — `CopyModeToolbar` stacked on top of `SshKeyboardBar` when copy mode is active, just the keyboard bar otherwise. `bottomStripHeight` is a fixed constant (`itemHeightXl + copyToolbarHeight` in copy mode, else `itemHeightXl`) and **does not track the keyboard inset**. That's the central invariant: the terminal's widget height is constant across soft-keyboard open/close, so xterm never reflows its buffer on keyboard show/hide and the scrollback-duplication regression from commit 20c60c9 stays fixed. The bottom strip does float with `viewInsets.bottom`, so the toolbar + keyboard bar dock flush against the soft keyboard; the keyboard physically covers the last couple of terminal rows, which is the same tradeoff the pre-copy-mode layout already made. A minor reflow *does* happen when copy mode toggles — `bottomStripHeight` grows by the toolbar slot — but that is a per-mode-switch event, not a per-keyboard-event storm. Hint + toolbar are regular Column children, not `Positioned` overlays, so they **shrink** the terminal area instead of floating on top of its last rendered row — covering the active line is unusable on a phone-sized viewport. The whole column lives inside a `Padding(EdgeInsets.only(bottom: viewInsets.bottom))` that pushes it above the soft keyboard, so the toolbar + SSH keyboard bar butt flush against the virtual keyboard with no phantom gap. `TerminalCopyOverlay` itself renders only the virtual cursor marker (wrapped in `IgnorePointer` so the outer `Listener` still sees cursor-pan deltas); on enter it calls `TerminalController.setSuspendPointerInput(true)`. One-finger drags are routed through `TerminalCopyOverlayState.onCursorPan(delta)`, which accumulates sub-cell pixel deltas against the measured cell size and advances the virtual cursor one cell at a time; when the cursor would step past the top / bottom viewport edge, `MobileTerminalView`'s shared `ScrollController` (passed to both `TerminalView(scrollController: ...)` and `TerminalCopyOverlay`) is nudged by the overflow cells' worth of pixels instead — so a single drag can extend the selection through the entire scrollback, not just one screen. The overlay derives the viewport-start line from `scrollController.offset ~/ cellHeight` so selection anchors stay buffer-absolute and accurate while the buffer scrolls under the cursor. Horizontal overflow still clamps — xterm has no horizontal scrollback. The *first* pointer-down in a session calls `onAnchorDown()`, which stamps the current cursor cell as the buffer-absolute selection anchor via `buffer.createAnchor(x, y)` + `TerminalController.setSelection`. Subsequent cursor movements extend the selection from the anchor to the new cursor position. The toolbar exposes **Copy** (routes to `TerminalClipboard.copy`, auto-exits copy mode via `SshKeyboardBarState.exitCopyMode()`) and **Cancel** (exits without copying). Paste stays on the keyboard bar as its own button so the user doesn't have to enter copy mode to paste a password into the shell — the two directions are orthogonal.

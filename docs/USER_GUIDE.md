...
- **Connection section** ([§3 — see protocol notes](#3-authentication)): per-protocol transport fields — SSH exposes Host (required) + Port (default 22) on one row and Username (required) below, each validated per field. WebDAV exposes base URL + username; S3 exposes access key id / region / endpoint / addressing / default bucket / prefix.

...
| `core/security/biometric_auth.dart` | 314 | partially done — Linux fully migrated via `FprintdClient` (Tier 2 `lfs_core::platform::linux::fprintd`). Apple / Windows / Android remain on `local_auth` because **(a)** Apple LAContext needs Cocoa block-callback → Rust Future bridging via `block2` (the lifetime / completion-handler dance is a focused arc by itself); **(b)** Windows Hello via `windows::Security::Credentials::KeyCredentialManager` needs a real Windows runtime to verify; **(c)** Android BiometricPrompt is a JNI bridge that needs Java-side hooks. None are blockers for the Tier 3 desktop cipher policy — they're UX surfaces that route to the *same* Apple SecAccessControl / WinHello-protected vault the storage layer already binds. |

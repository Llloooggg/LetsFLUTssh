...
- [-] **5.1 Connection lifecycle → Rust actor (in progress)**

  **Shipped:** `lfs_core::connection::ConnectionRegistry` + `ConnectionActor` types,
  `ConnectionState` / `ConnectionPhase` / `StepStatus` / `ProgressStep` mirrors of the
  Dart enums, `ConnectionSnapshot` plain-data view, FRB mirrors of all of the above,
  `bus::Event::Connection*` variants + `bus::Command::ConnectionDisconnect`. The
  `connect_async` driver loop is implemented inside `lfs_core::connection` but
  not yet exposed over FRB.

  **Blocker:** wiring `connect_async` through FRB hits a notorious rustc
  HRTB inference bug — the future generated by chaining `wrap_async →
  connect_async → Session::connect_*_with_secret(&str, …)` fails
  `Send + 'static` ("`Send` is not general enough" against `&str` for an
  unspecified lifetime). The fix is to add owned-arg variants to
  `lfs_core::ssh::Session::connect_*_with_secret` (signatures take `String` /
  `Vec<u8>` instead of `&str` / `&[u8]`) so the FRB future never closes
  over a borrow that propagates the auto-trait conditional bound. Once that
  surface lands, the `bus::Command::ConnectionConnect` variant + matching
  FRB entry come back.

  **Remaining scope (this sub-phase):** owned-arg connect variants in
  `lfs_core::ssh`; bus + FRB wiring for `ConnectionConnect`; ProxyJump
  bastion-aware dispatch; Dart-side `Connection` view-model rework; consumer
  updates (workspace, terminal, sftp); test refactor.

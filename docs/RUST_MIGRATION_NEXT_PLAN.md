...
| 11. `qr_codec` finish + `import_service` close | partial — Dart QR decode fallback path retired (paste-link dialog + import-flow + QrDecodedSource sealed-class branch). Rust `qrImportOpen` is the only production decode entry point. The standalone `QrExportDialog` was deleted as dead code (production already routed through `UnifiedExportDialog` → `dbExportQrPayload`); `wrapInDeepLink` retired with it. `lfs_core::qr_codec_encode` now owns the deflate + base64url-no-pad transport encoding step shared by every producer (the Rust archive exporter consolidated into the same helper, the Dart in-memory encoder + the size-estimator route through the FRB shim with a `package:archive` Dart fallback for flutter_test). `import_service::_stageFromResult` (OpenSSH-config import path) + the JSON-shape composition inside `encodeExportPayload` (Dart Session→JSON marshalling) remain pending. |

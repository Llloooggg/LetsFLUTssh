...
| 4. `connection_manager` → Rust | partial — `lfs_core::connection::ConnectionRegistry` already owns the actor + state machine + ProxyJump; `connected_user_visible_count` + `ConnectionActiveCountChanged` bus event added so Rust is the source of truth for the foreground-service active-count gate. Dart `ConnectionManager` registry mirror + `_credentialCache` Dart wrapper retire pending. |

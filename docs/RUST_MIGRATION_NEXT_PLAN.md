...
| 4. `connection_manager` → Rust | partial — `lfs_core::connection::ConnectionRegistry` already owns the actor + state machine + ProxyJump; `connected_user_visible_count` + `ConnectionActiveCountChanged` bus event added so Rust is the source of truth for the foreground-service active-count gate. `SessionCredentialCache` shrunk to the write-through path (store / evict / evictAll) — the dead `read()` accessor + `CachedCredentials` no-op stub retired together with the Dart-side `_overlaySessionCache` branch in `ConnectionManager`. Dart `ConnectionManager` registry mirror retire still pending. |
| 5. `transfer_manager` → Rust driver | DONE — Rust executor (chunked streaming + cancellation) + full FRB enqueue / cancel / snapshot / drop / clear surface owns scheduling and per-task state. Dart `TransferManager` shrank to a snapshot-mirror over the `Transfer` bus topic; `TransferTask` closure model retired. Directory transfers walk Dart-side and enqueue per-leaf-file Rust tasks (UX trade-off: per-file rows in the panel). |
...
| 7. `known_hosts` manager → Rust + prompt protocol | DONE — `lfs_core::known_hosts` owns import/export + change-event publishing + the TOFU prompt registry; russh's `check_server_key` consults the DB, fires `KnownHostPromptRequest` on unknown/changed keys, awaits a `oneshot` resolved by `KnownHostPromptResponse`, and persists accepted entries before resuming the handshake. Dart `HostKeyPromptListener` subscribes to the bus + surfaces the host-key dialog. |
...
| 9. Security tier stack → Rust | partial — every persisted artefact in the security-tier stack now has its on-disk format owned Rust-side and routed via the `_crypto_compat.dart` Rust-canonical / Dart-fallback wrappers. Inventory: `lfs_core::security::{SecurityTier, SecurityTierModifiers, SecurityConfig}` typed scaffold + JSON ser/de mirroring Dart's permissive fallback (Dart `SecurityConfig.toJson` / `fromJson` route through the shim); `SecurityCapabilities` JSON wire format owned by `lfs_core::security::capabilities`; `lfs_core::rate_limit::InMemoryRateLimiterRegistry` (Dart shim); `lfs_core::security::master_password` owns `credentials.kdf` / `credentials.verify` (Argon2id derive + AES-GCM verifier + 0600 perm hardening); `lfs_core::security::keychain_marker` owns the libsecret-noise gate marker file; `lfs_core::keys::is_encrypted_pem` owns the PEM encryption-format scan; `lfs_core::security::wipe` owns the catastrophic-reset file catalogue + `.wipe-pending` crash marker + per-file sweep + `logs/` wipe; `lfs_core::security::keychain_password_gate` owns the L2 disk-blob `{salt, hmac}` JSON envelope; `lfs_core::security::hardware_tier_vault` owns the L3 Linux `{salt, sealed}` JSON envelope + the (password, biometric) → auth-bytes resolver shared across the Linux TPM + per-platform method-channel plugins; `lfs_core::security::persisted_rate_limit` owns the HMAC-authenticated `{payload, hmac}` rate-limit state frame; `lfs_core::security::tier_transition_marker` owns the `.tier-transition-pending` crash-recovery marker; `lfs_core::security::map_wizard_choice` owns the wizard radio-set → `SecurityTier` + typed-secret-slot mapping. Cross-cutting crypto primitives consolidated in `lfs_core::crypto` and routed through `_crypto_compat.dart`: `hmac_sha256`, `constant_time_eq` (subtle-backed), `sha256` / `sha256_hex`, `looks_sensitive` heuristic. Hardware-tier composer + keychain gate runtime + biometric vault + tier state-machine actor + bootstrap orchestration still Dart-side because each rides on platform plugins (`flutter_secure_storage`, `MethodChannel`) without an `lfs_core` equivalent. |
...
| 11. `qr_codec` finish + `import_service` close | partial — Dart QR decode fallback path retired (paste-link dialog + import-flow + QrDecodedSource sealed-class branch). Rust `qrImportOpen` is the only production decode entry point. The standalone `QrExportDialog` was deleted as dead code (production already routed through `UnifiedExportDialog` → `dbExportQrPayload`); `wrapInDeepLink` retired with it. `lfs_core::qr_codec_encode` now owns the deflate + base64url-no-pad transport encoding step shared by every producer (the Rust archive exporter consolidated into the same helper, the Dart in-memory encoder + the size-estimator route through the FRB shim with a `package:archive` Dart fallback for flutter_test); `lfs_core::qr_codec_encode::encode_session_compact` owns the v4 per-session map shape (`l`/`h`/`u`/`p`/`g`/`a`/`ki`/`mg`/`pw` + default-omit grammar) — Dart `encodeSessionCompact` routes through the FRB shim; the same helper backs the DB-side `qr_export_payload` writer so the field-name contract lives one place. `lfs_core::archive_stage` owns the in-memory import staging shape — typed `StagedSessionImport` / `StagedKeyImport` / `StagedTagImport` / `StagedSnippetImport` + per-type JSON-string serialisers; Dart `import_service::_stageFromResult` calls the matching `archive_stage_*_to_json` FRB shims so the JSON contract the apply driver parses lives one place across the in-memory + `.lfs` paths. |
...
| `core/security/password_rate_limiter.dart` | ~440 | exponential backoff state — `PersistedRateLimiter` HMAC-authenticated disk frame (`{payload, hmac}` JSON envelope; inner `{failure_count, next_retry_at_millis}`) lives in `lfs_core::security::persisted_rate_limit`. Dart still owns the file I/O, the in-memory cache, and the `InMemoryRateLimiter` shim over `lfs_core::rate_limit::InMemoryRateLimiterRegistry`. | 9 |
| `core/security/hardware_tier_vault.dart` | ~390 | TPM / Keychain / WinBio composer — Linux disk-blob format (`{salt, sealed}` JSON envelope) lives in `lfs_core::security::hardware_tier_vault`; HMAC compose already routes through the shared crypto compat. Dart still owns TPM CLI shell-out, method-channel calls into the per-platform native vault plugin, and the salt-file persistence for the method-channel platforms. | 9 |
...
**Pure helpers landed ahead of the actor arc.** Three Dart-side

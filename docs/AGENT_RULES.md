...
1. **TOC → specific §, never cover-to-cover.** `ARCHITECTURE.md` is 3000+ lines; reading it whole wastes context and pushes the detail you need out of working memory. The strict order is:
   - **(a) Consult the table of contents first** — the [Within ARCHITECTURE.md nav](#within-architecturemd) in this file, your tool's Action → Read table if it has one, or the TOC at the top of `docs/ARCHITECTURE.md` itself. The TOC is the index, not optional scaffolding.
...
4. **After your edits, walk the [Documentation Maintenance Checklist](#documentation-maintenance-checklist) below.** Find every row the diff triggers and update the named §. Ship the doc update in the same commit as the code change — a code change without its matching doc update is an incomplete commit.
...
6. **When you rename, move, merge, split, or delete a § (or anything with an anchor), update every inbound link in the same commit.** A broken anchor is a stale rule — the link target the reader clicked no longer exists, and whatever rule the link was enforcing silently stops working. Before committing a § restructure:
   ...
   - **Check each of these surfaces** and fix every match:
     ...
     - Any tool-specific agent-instruction files the project carries (Documentation Map, Action → Read table, Always-On Rules indexes) that link into this file or `ARCHITECTURE.md` — sweep those too.
   ...
   This step closes the "I renamed a § and now three files silently link to an empty anchor" failure mode. It applies equally to renaming a § in `ARCHITECTURE.md`, a rule in `AGENT_RULES.md`, a bullet in any tool-specific agent file, or a file itself (moved `docs/X.md` → update everything that links to the old path).
...
| Architecture changed | Update tool-specific agent-instruction files if their navigation links into the renamed/moved §s |
...
- *"`secret_buffer.dart` already does `mlock` via `dart:ffi`."* No. "Already works" is the floor, not the ceiling. The pillars push toward consolidating libc FFI inside `lfs_os_security` so the unsafe surface lives in one auditable crate.
...
This rule overrides the standard "don't add features beyond what the task requires" guidance (carried in your tool's default instructions and in [§ Don't Escalate Working Baselines](#dont-escalate-working-baselines)) for migration / refactor work specifically. When the user is in pillars-mode, aggressive completionism is the right posture.
...
**3. No fabricated rationale.** Only cite platforms, measurements, failure mechanisms, or behavioural claims that are real and verifiable from the code, the git log, or a captured user report. Forbidden shapes:

- Concrete timings without a measurement source: `~3 s on Win IoT`, `~5 s of dbInit on Windows IoT`, `~500 ms on healthy hosts`. Don't substitute generic-sounding alternatives either (`non-trivial latency`, `measurable wait`, `takes time to load`) — those are still timing claims dressed up; they imply you measured something. If you didn't, drop the timing dimension entirely and document the structural reason instead (the bug class the invariant prevents, the API contract it enforces).
- Specific platforms named as the canonical case for a generic constraint when the project's CI / supported-platforms list does not enumerate the platform. Confirm the platform exists in `Makefile` / CI workflows / `pubspec.yaml` targets before naming it.
- Specific OS subsystems pinned as the cause of a slowdown without a concrete link (`Defender real-time scan`, `Gatekeeper signature check`, `SELinux relabel`). Generic `the OS may inspect new binaries` is fine; pinning the subsystem fabricates a falsifiable claim.
- Causal chains that explain *why* a thing was done a particular way when you have only the *what*. If the deferral / refactor / split exists in the code but the "why" is not in the commit message, the docs, or a user-confirmed bug, document the structural fact and the invariant it enforces — do not invent the rationale.

Invariants survive on their own merits — pointing at the bug class they prevent or the contract they enforce is enough. The check before shipping a specific: would the reader's "how was this measured / where does this come from" find a source, or just my own confidence?

Same rule applies to Rust `//`/`///`/`//!` doc comments and to Dart `///` doc comments — short, current, no retrospective, no fabricated specifics. Multi-paragraph module-level `//!` docstrings get the same `ARCHITECTURE.md`-link treatment.
...
**No debug / verbose rung.** The taxonomy stops at `info` on purpose — adding a fourth level would mean one more dropdown row, one more filter chip, and "what counts as debug?" bikeshedding on every review. If a single call site genuinely needs per-packet / per-frame tracing to chase a specific bug, wrap it behind a local flag you ship and revert — don't grow the enum for it. Adding a level back is one commit when a real need appears.
...
**Picking info vs warn — the test.** "Would a user pasting 20 lines from the viewer into a bug report want this line in those 20?" If yes and the operation succeeded → `info`. If the operation degraded → `warn`. Per-packet / per-frame / per-item-in-a-hot-loop should not land in the log at all — if it's that noisy, it doesn't belong on disk for a user-facing build.

**All levels are user-visible.** Once the user picks any threshold, every line at or above lands in the same file they can export to a bug report. Write every I / W / E line as if it will be read out of context: full noun, short verb, concrete subject. No debug-noise carve-out, no "only the dev sees this" assumption.

**Dev / beta-tester builds override the threshold at compile time.** `--dart-define=LETSFLUTSSH_LOG_LEVEL=<level>` (value is the same string as `config.behavior.log_level`: `info` / `warn` / `error`) seeds the threshold before `config.json` is read, so a freshly-installed dev build already writes lines without the tester having to open Settings. `make run` wires this to `info` by default (everything lands on disk). Release builds ship **without** the flag — `buildTimeLogLevelOverride` returns null and the on-disk config wins. Never ship a release with the flag set: that would override every user's chosen level silently.
...
- `README.md`, `ARCHITECTURE.md`, `SECURITY.md`, `AGENT_RULES.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, and any tool-specific agent-instruction file the project carries
...
- **Failing tests after a code change: diagnose before editing the test.** When an existing test goes red after your refactor / fix / optimization, the default reaction must be to read the test and ask "is this test catching a regression my change just introduced?" — not "how do I update the assertions to match the new output?" Blindly rewriting `expect(...)` to match whatever the code now emits launders bugs into tests: the next contributor sees a green suite and trusts the previous behavior is covered, when in fact you deleted the coverage. Triage in this order: (1) re-derive the test's intended contract from its name, setup, and any docstring; (2) compare to what your change actually promised to do; (3) if the test is still checking a contract your change did **not** intend to break, the code is wrong — fix the code; (4) only when the test is pinning an internal detail you deliberately reshaped (e.g. it subscribed to the wrong notifier, asserted an intermediate data structure, or locked in a behavior the feature spec explicitly widened) do you rewrite the assertion, and the new assertion must express the **new** contract in the same intent-first style — not a paste of the current output. If you cannot tell which bucket the failure falls into, stop and report — same surface-the-disagreement discipline as the bullet above.

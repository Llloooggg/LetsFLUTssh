...
**Example where the escape hatch (moving makes it worse) legitimately fires** — the `single_instance` revert, 2026-05-04: migrated to `lfs_os_security::single_instance` per this rule, then reverted to pure Dart. Reason: the FRB call needed `RustLib.init()` first, but the boot order paints the splash before that load, so the lock check has to run before `RustLib.init` — which threw "not initialised" and routed every solo launch to `AlreadyRunningApp`. Pillar 1's "moving makes the system *worse*" triggered (measurable functional regression). See [`RUST_CORE_MIGRATION_PLAN.md` § Reverted: single_instance](RUST_CORE_MIGRATION_PLAN.md#reverted--single_instance-back-to-dart). The exception is real; the bar is "concrete regression we can point at", not "feels iffy".

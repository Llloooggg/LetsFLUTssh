...
- **Text selection is opt-in on desktop.** No global `SelectionArea` wraps the desktop shell — an earlier global wrap broke `ThresholdDraggable` because `SelectionArea`'s `TapAndDragGestureRecognizer` claimed pan ahead of `MultiDragGestureRecognizer`. Wrap specific prose surfaces in `AppSelectionArea` locally: dialog bodies (already wrapped inside `AppDialog`), threat lists, release-notes bodies, help prose. Never wrap a container that also hosts a drag target, `AppButton`, or interactive row. Mobile keeps one `AppSelectionArea(child: MobileShell())` — touch drag recognisers arbitrate differently. Inside a scoped `AppSelectionArea`, block selection on subtrees that are not `HoverRegion`-wrapped via explicit `SelectionContainer.disabled` (see `settings_screen.dart` sidebar). `HoverRegion` auto-wraps its child in `SelectionContainer.disabled` when a gesture callback is bound — [§6 Widgets API → Selection scoping](ARCHITECTURE.md#selection-scoping)

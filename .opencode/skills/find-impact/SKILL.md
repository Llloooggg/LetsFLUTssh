...
1. **Paired test file** — mirror path under `test/`. In this repo the rule is one test per source file (see AGENTS.md "Testing Methodology").
...
2. **Find paired test.**
   ...
   - Report whether the test exists; if not, flag it — per AGENTS.md "Testing Methodology" (target 100% coverage).
